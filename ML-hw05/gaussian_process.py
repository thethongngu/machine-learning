...
def kernel(x, y, variance, alpha, length):
    return variance * ((1 + cdist(x, y, 'sqeuclidean') / (2 * alpha * length)) ** -alpha)
...
upper_y = mu_y + 2 * (np.diag(np.sqrt(variance_y)).reshape(-1, 1))
lower_y = mu_y - 2 * (np.diag(np.sqrt(variance_y)).reshape(-1, 1))
